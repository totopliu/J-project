...
import com.crm.framework.mybatis.dao.MybatisSqlDao;
...
public class ListTag extends BodyTagSupport implements DynamicAttributes {
    ...
    public int doStartTag() throws JspException {
        try {
            ...
            // MybatisSqlDao mybatisSqlDao = (MybatisSqlDao)
            // appCtx.getBean(MybatisSqlDao.class);
            MybatisSqlDao mybatisSqlDao = (MybatisSqlDao) AppContextUtil.getBean("mybatisSqlDaoImpl");
            // 获取对应的service 调用查询方法 返回结果给LIST
            list = (List<?>) mybatisSqlDao.queryForList(namespace,
                    StringUtils.isBlank(param) ? null : JSONObject.parseObject(param));
            if (list.isEmpty())
                return SKIP_BODY;
