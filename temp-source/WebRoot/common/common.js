...
(function() {
	...
	crm.ajaxJson = (function(options,callback) {
		var dataType = options.dataType == undefined  ? 'json' : options.dataType;
		if(options.url == undefined)
		{
			layer.alert("请输入crm.ajax的URL地址");
			return;
		}
		var url = options.url;
		if (url.indexOf("?") > -1) {
	...
	crm.ajaxSubmit = (function(form, params) {// form 表单ID. params ajax参数
		var pp = {
			error : function(XMLHttpRequest, textStatus, errorThrown) {
				layer.open({
					type : 1,
	...
	crm.showWindow = (function(options) {
		...
		if(type == 1)
		{
			$.get(href,{},function(str){
				var index = layer.open({
					...
					content : str,
					btnAlign: 'c',
					btn : [$.i18n.prop('stringSubmit'), $.i18n.prop('stringCancel')],
...
//				initUI();
	 ...
	 * 增加命名空间功能
	 * 
	 * 使用方法：crm.ns('jQuery.bbb.ccc','jQuery.eee.fff');
	 */
	crm.ns = function() {
		var o = {}, d;
	...
	crm.msg = function (msg) {
		layer.msg(msg);
	}
	
	crm.toastrsAlert = function(obj){
		crm.init();
		if(obj.code == 'info'){
	...
	//关闭弹出窗口
	crm.closeWindow = function(){
			layer.closeAll();
	}
	//关闭弹出子窗口
	crm.closeNestWindow = function(index){
			layer.close(index)
	}
	...
	crm.confirm = function confirm(dowork){
		layer.confirm($.i18n.prop('stringWantContinue'), {title:$.i18n.prop('stringPrompt')}, function(index){
			dowork();
			layer.close(index);
		});
	}
	...
	CommnUtil = {
		...
		ajax : function(url, data, dataType) {
			...
			var html = '没有结果!';
			// 所以AJAX都必须使用crm.ajax..这里经过再次封装,统一处理..同时继承ajax所有属性
			if (url.indexOf("?") > -1) {
				url = url + "&_t=" + new Date();
			} else {
				url = url + "?_t=" + new Date();
